...
function custom_thumbnail($post_id, $size = 'thumbnail')
{
	if (has_post_thumbnail($post_id)) {
		$image_src = wp_get_attachment_image_src(get_post_thumbnail_id($post_id), $size);
		$image_alt = get_post_meta(get_post_thumbnail_id($post_id), '_wp_attachment_image_alt', true);
		$image = '<img src="' . esc_url($image_src[0]) . '" alt="' . esc_attr($image_alt) . '" />';
		return $image;
	}
	return false;
}
...
$thumbnail = custom_thumbnail($post_id, 'medium'); // Get the medium-sized thumbnail
if ($thumbnail) {
...
/* End of Breadcrumb */
function remove_page_titles($title, $id = null)
{
	if (is_page($id)) {
		return '';
	}
